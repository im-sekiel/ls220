...
function nexusSurge(actions) {
  ...
  actions.forEach(char => {
    switch(char) {
    case '+':
      last = scoreboard.length - 1;
      sec = last - 1;
      scoreboard.push(scoreboard[last] + scoreboard[sec]);
      break;
    case '-':
      scoreboard.pop();
      break;
    case '*':
      ...
      break;
    default:
      scoreboard.push(Number(char));
      break;
    }
  });

  console.log(scoreboard);
  return scoreboard.reduce((acc, num) => acc += num, 0);
}
